...
def test_extract_successfully_read_csv_file(
    ...
    assert test_data_unit_of_work.file.file_exists(test_const.CORRECT_INPUT_CSV)
    assert len(test_data_unit_of_work.extract.list()) == 1  # type: ignore[attr-defined]
    assert (
        test_data_unit_of_work.extract.list()[0].input_file_name  # type: ignore[attr-defined]
        == test_const.CORRECT_INPUT_CSV.name
    )
    assert (
        test_data_unit_of_work.extract.list()[0].saved_file_name  # type: ignore[attr-defined]
        == test_const.CORRECT_INPUT_CSV.name
    )
...
def test_extract_successfully_read_xlsx_file(
    ...
    assert not input_data.empty  # type: ignore[attr-defined]
    assert input_data.shape[0] == test_dataset_size  # type: ignore[attr-defined]
    assert test_data_unit_of_work.file.file_exists(test_const.CORRECT_INPUT_XLSX)
    assert len(test_data_unit_of_work.extract.list()) == 1  # type: ignore[attr-defined]
    assert (
        test_data_unit_of_work.extract.list()[0].input_file_name  # type: ignore[attr-defined]
...
def test_extract_successfully_read_xls_file(
    ...
    assert not input_data.empty  # type: ignore[attr-defined]
    assert input_data.shape[0] == test_dataset_size  # type: ignore[attr-defined]
    assert test_data_unit_of_work.file.file_exists(test_const.CORRECT_INPUT_XLS)
    assert len(test_data_unit_of_work.extract.list()) == 1  # type: ignore[attr-defined]
    assert (
        test_data_unit_of_work.extract.list()[0].input_file_name  # type: ignore[attr-defined]
