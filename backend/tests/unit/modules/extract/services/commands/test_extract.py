...
def test_extract_successfully_read_csv_file(
    ...
    input_data: value_objects.InputData = extract(
        ...
        command=commands.ExtractData(
            file=const.CORRECT_INPUT_CSV.read_bytes(),
            file_name=const.CORRECT_INPUT_CSV.name,
        ),
    )

    # Then
    assert not input_data.empty  # type: ignore[attr-defined]
    assert len(cast(pd.DataFrame, input_data)) == const.DATASET_INPUT_SIZE

    assert test_extract_unit_of_work.file.file_exists(const.CORRECT_INPUT_CSV.name)  # type: ignore[attr-defined]

    assert len(test_extract_unit_of_work.extract.list()) == 1  # type: ignore[attr-defined]
    assert all(
        extract_history.input_file_name == const.CORRECT_INPUT_CSV.name
        and extract_history.saved_file_name == const.CORRECT_INPUT_CSV.name
...
def test_extract_successfully_read_xlsx_file(
    ...
    input_data: value_objects.InputData = extract(
        extract_unit_of_work=test_extract_unit_of_work,
        command=commands.ExtractData(
            file=const.CORRECT_INPUT_XLSX.read_bytes(),
            file_name=const.CORRECT_INPUT_XLSX.name,
        ),
    )
    # Then
    assert not input_data.empty  # type: ignore[attr-defined]
    ...
    assert test_extract_unit_of_work.file.file_exists(const.CORRECT_INPUT_XLSX.name)  # type: ignore[attr-defined]
    ...
    assert all(
        extract_history.input_file_name == const.CORRECT_INPUT_XLSX.name
        and extract_history.saved_file_name == const.CORRECT_INPUT_XLSX.name
...
def test_extract_successfully_read_xls_file(
    ...
    input_data: value_objects.InputData = extract(
        extract_unit_of_work=test_extract_unit_of_work,
        command=commands.ExtractData(
            file=const.CORRECT_INPUT_XLS.read_bytes(),
            file_name=const.CORRECT_INPUT_XLS.name,
        ),
    )
    # Then
    assert not input_data.empty  # type: ignore[attr-defined]
    ...
    assert test_extract_unit_of_work.file.file_exists(const.CORRECT_INPUT_XLS.name)  # type: ignore[attr-defined]

    extract_history_results = test_extract_unit_of_work.extract.list()  # type: ignore[attr-defined]
    assert len(extract_history_results) == 1
    assert all(
        extract_history.input_file_name == const.CORRECT_INPUT_XLS.name
        and extract_history.saved_file_name == const.CORRECT_INPUT_XLS.name
        for extract_history in extract_history_results
    )
...
def test_extract_raise_exception_when_file_type_is_not_supported(
    ...
    extract_command = commands.ExtractData(
        file=const.NOT_SUPPORTED_INPUT.read_bytes(),
        file_name=const.NOT_SUPPORTED_INPUT.name,
    )
...
def test_extract_raise_exception_when_any_dataset_row_is_invalid(
    ...
    # Given
    extract_command = commands.ExtractData(
        file=const.INCORRECT_INPUT.read_bytes(),
        file_name=const.INCORRECT_INPUT.name,
    )
